...
interface PhysioProfile {
  id: string; full_name: string; physio_region: string; physio_status: string; clinic_name: string; physio_rate: number; avatar_url: string | null;
}
...
export default function PhysioPage() {
  ...
  const uniqueLocations = Array.from(new Set(physios.map(p => p.physio_region).filter(Boolean))) as string[];
  ...
  const filteredPhysios = physios.filter(p => {
    ...
    const matchLocation = selectedLocations.length === 0 ? true : selectedLocations.includes(p.physio_region);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 lg:gap-8 animate-fadeIn">
              {filteredPhysios.map((p) => (
                <div key={p.id} className="bg-slate-900/50 border border-slate-800 hover:border-slate-600 rounded-2xl p-6 flex flex-col items-center text-center transition duration-300 group hover:-translate-y-1 shadow-md hover:shadow-xl relative overflow-hidden">
                  ...
                  <p className="text-xs md:text-sm text-zinc-400 font-medium mb-5 line-clamp-2 h-8 md:h-10 leading-snug">{p.clinic_name || "獨立接案"}</p>
                  <div className="flex flex-wrap items-center justify-center gap-2 mb-6 w-full">
                    <div className="bg-slate-950/50 border border-slate-800/80 text-zinc-400 text-xs font-bold px-3 py-1.5 rounded-lg truncate max-w-[140px]">📍 {p.physio_region || "地點未設"}</div>
                    <div className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-black px-3 py-1.5 rounded-lg truncate">HK$ {p.physio_rate || 0} / 次</div>
                  </div>
                  <div className="mt-auto w-full pt-4 border-t border-slate-800/80">
                    <Link href={`/p/${p.id}`} className="block w-full bg-slate-800 hover:bg-emerald-600 text-white text-sm font-black py-3 rounded-xl transition duration-300">查看專頁</Link>
                  </div>
                </div>
              ))}
            </div>
