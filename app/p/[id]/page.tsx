...
const GALLERY_MEDIA: MediaItem[] = [
  ...
  },
  {
    id: "pub-2",
    sportName: "Volleyball",
    url: "https://images.unsplash.com/photo-1592656094267-764a45160876?q=80&w=800&auto=format&fit=crop",
    createdAt: "隊內分組賽",
  },
  {
    id: "pub-3",
...
const MOCK_SPORTS: UserSport[] = [
  {
    id: "us1",
    metadata: { position: "主攻手 Outside Hitter" },
    sports: { name: "Volleyball" },
  },
  {
    id: "us2",
    metadata: { position: "高級教練" },
    sports: { name: "Physical Training" },
  },
];
...
function StatusBadge({ tag }: { tag: string | null }) {
  ...
  return (
    <div className="inline-flex items-center gap-1.5 bg-pro-slate-800/50 border border-pro-slate-700/50 text-zinc-400 text-[10px] px-2.5 py-1 rounded-full font-black tracking-widest">
      <div className="w-1.5 h-1.5 rounded-full bg-pro-slate-500" />
      穩定訓練
    </div>
  );
}
...
export default function PublicPlayerPage() {
  ...
  const fetchData = useCallback(async () => {
    // Mock shortcut for test IDs
    if (targetUserId === "p1") {
      setProfile(MOCK_PROFILE);
      setUserSports(MOCK_SPORTS);
      setIsLoading(false);
      return;
    }

    // Real DB fetch for valid UUIDs
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 xl:col-span-3 space-y-6">
            <div className="bg-pro-slate-900/40 backdrop-blur-xl border border-pro-slate-800/60 rounded-3xl p-6 lg:sticky lg:top-20 shadow-2xl relative">
              ...
              <div
                className="w-32 h-32 mx-auto mb-6 mt-4 rounded-[2rem] bg-pro-slate-800 border-2 border-pro-slate-700/50 shadow-xl overflow-hidden flex items-center justify-center text-4xl font-black text-zinc-600 bg-cover bg-center"
                style={{
              ...
              <div className="text-center mt-2 space-y-3">
                ...
                <div className="flex flex-col gap-3 pt-2">
                  <button className="w-full bg-pro-slate-50 text-black font-black py-3 rounded-xl transition hover:bg-pro-slate-200">
                    ＋ 關注選手
                  </button>
                  ...
                  {userSports.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {userSports.map((us) => (
                        <div
                          key={us.id}
                          className="bg-pro-slate-900/50 border border-pro-slate-800 rounded-2xl p-5"
                        >
                          <span className="text-base font-black text-white">
                            🏆 {us.sports?.name}
                          </span>
                          <p className="text-xs text-blue-400 mt-1">
                            場上定位：{us.metadata?.position ?? "核心主力"}
                          </p>
                        </div>
                      ))}
                    </div>
                ...
                <div className="space-y-6 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {GALLERY_MEDIA.map((m) => (
                      <div
                        key={m.id}
                        className="aspect-square bg-pro-slate-800 rounded-2xl overflow-hidden relative border border-pro-slate-800 shadow-md"
                      >
                        <img
                          src={m.url}
                          alt={m.sportName}
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent p-3">
                          <span className="text-[10px] font-black text-white bg-pro-blue-600 px-2 py-0.5 rounded">
                            {m.sportName}
                          </span>
                        </div>
                      </div>
