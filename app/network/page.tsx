...
interface ProfileRow {
  ...
  bio: string | null; // 💡 加入 bio 讀取供中間小區塊使用
  avatar_url: string | null;
  status_tag: string | null;
  display_sports: string[] | null;
  height_cm: number | null; // <-- ADD THIS
  weight_kg: number | null; // <-- ADD THIS
  show_physical_stats: boolean | null; // <-- ADD THIS
...
function PlayerStatusBadge({ tag }: { tag: string | null }) {
  if (tag === "recruiting")
    return (
      <div className="inline-flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] md:text-xs px-2.5 py-1 rounded-full font-black tracking-widest whitespace-nowrap shadow">
  ...
  if (tag === "seeking_team")
    return (
      <div className="inline-flex items-center gap-1.5 bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] md:text-xs px-2.5 py-1 rounded-full font-black tracking-widest whitespace-nowrap shadow">
  ...
  if (tag === "open_to_match")
    return (
      <div className="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] md:text-xs px-2.5 py-1 rounded-full font-black tracking-widest whitespace-nowrap shadow">
        <div className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />{" "}
        開放約戰
      </div>
    );
  return (
    <div className="inline-flex items-center gap-1.5 bg-slate-800/80 border border-slate-700/50 text-zinc-400 text-[10px] md:text-xs px-2.5 py-1 rounded-full font-black tracking-widest whitespace-nowrap shadow">
    ...
    const fetchData = async () => {
      ...
      if (!profilesRes.error && profilesRes.data) {
        const formattedProfiles = profilesRes.data.map((p: any) => ({
          ...p,
          all_sport_names:
            p.user_sports?.map((us: any) => us.sports?.name).filter(Boolean) ||
            [],
        }));
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 p-4 md:p-5 rounded-3xl mb-8 shadow-lg flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 [&::-webkit-scrollbar]:hidden w-full md:w-auto">
            <button
              ...
              className={`whitespace-nowrap px-4 py-2.5 rounded-xl text-xs font-bold border transition cursor-pointer ${
                !filterStatus
            ...
            <button
              onClick={() => setFilterStatus("seeking_team")}
              className={`whitespace-nowrap px-4 py-2.5 rounded-xl text-xs font-bold border transition cursor-pointer ${
                filterStatus === "seeking_team"
            ...
            <button
              onClick={() => setFilterStatus("open_to_match")}
              className={`whitespace-nowrap px-4 py-2.5 rounded-xl text-xs font-bold border transition cursor-pointer ${
                filterStatus === "open_to_match"
                  ? "bg-slate-100 border-slate-200 text-black"
                  : "bg-slate-950 border-slate-700 text-zinc-400 hover:border-slate-500"
              }`}
            >
        ...
        <div>
          <div className="mb-4 px-1 flex justify-between items-center">
            <span className="text-sm font-bold text-zinc-500">
              顯示 <span className="text-white">{filteredProfiles.length}</span>{" "}
              位運動員
            </span>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 lg:gap-8 animate-fadeIn">
              {filteredProfiles.map((p) => (
                <div
                  key={p.id}
                  className="bg-slate-900/60 border border-slate-800 hover:border-slate-700 rounded-3xl p-6 flex flex-col justify-between transition duration-300 group hover:-translate-y-1 shadow-md hover:shadow-2xl relative overflow-hidden"
                >
                  {/* 🔥 左上角：教練標籤 */}
                  {p.is_coach && p.coach_status !== "hidden" && (
                    <span className="absolute top-4 left-4 z-10 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-amber-500/15 text-amber-400 border border-amber-500/30 shadow-sm flex items-center gap-1">
                      🎓 教練
                    </span>
                  )}
                  ...
                  {p.is_physio && p.physio_status !== "hidden" && (
                    <span className="absolute top-4 right-4 z-10 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 shadow-sm flex items-center gap-1">
                      ⚕️ 物理治療
                    </span>
                  )}
                  ...
                  <div className="flex flex-col items-center text-center mt-4">
                    ...
                    <div className="relative w-20 h-20 md:w-24 md:h-24 mb-4">
                      <div
                        className="w-full h-full rounded-2xl bg-slate-800 border-2 border-slate-700/60 flex items-center justify-center text-3xl font-black text-zinc-600 overflow-hidden bg-cover bg-center shadow-lg"
                        style={
                          p.avatar_url
                    ...
                    {/* 姓名與頭銜 */}
                    <h3 className="text-lg font-black text-white group-hover:text-blue-400 transition mt-1 truncate max-w-full">
                      {p.full_name || "運動員"}
                    </h3>
                    <p className="text-xs md:text-sm text-blue-400 font-medium truncate max-w-full mb-2">
                      {p.headline || "熱愛運動與交流"}
                    </p>
                    
                    {/* 🔥 NEW: Physical Stats Badge (Only if opted in and data exists) */}
                  {p.show_physical_stats && (p.height_cm || p.weight_kg) && (
                    ...
                    {/* 地區標籤 */}
                    {p.location && (
                      <div className="inline-flex items-center gap-1 text-[11px] text-zinc-400 bg-slate-950/60 px-3 py-1 rounded-full border border-slate-800/80 mb-3">
                        <span>📍</span>{" "}
                    ...
                    {/* 🔥 中間精簡自介區塊 (Bio Section) */}
                    <p className="text-xs text-zinc-300 line-clamp-2 min-h-[2.5rem] leading-relaxed px-2 my-1">
                      {p.bio || "這位運動員很低調，尚未填寫自介。"}
                    </p>
                  </div>

                  <div className="space-y-4 pt-4 mt-2 border-t border-slate-800/80">
                    {/* 🔥 僅保留運動專項標籤 (左上與右上已顯示過教練/物理治療，此處不再重複呈現) */}
                    <div className="flex flex-wrap justify-center gap-1.5 min-h-[1.75rem]">
                      {p.display_sports && p.display_sports.length > 0 ? (
                        p.display_sports.slice(0, 3).map((sport: string) => (
                          <span
                            key={sport}
                            className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-blue-950/40 text-blue-300 border border-blue-500/20"
                          >
                            {sport}
                          </span>
                        ))
                    ...
                    <Link
                      href={`/p/${p.id}`}
                      className="block w-full py-2.5 rounded-xl bg-slate-800 hover:bg-blue-600 text-zinc-200 hover:text-white text-xs font-black text-center transition duration-200 shadow active:scale-95"
                    >
                      查看完整檔案
                    </Link>
                  </div>
