...
const PlayerDirectory = dynamic(() => Promise.resolve(PlayerDirectoryComponent), {
  ssr: false,
});
    ...
    <div className="relative w-full" ref={containerRef}>
      ...
      <button
        ...
        className={`w-full bg-pro-slate-900 border rounded-lg text-sm p-3 text-slate-200 text-left flex items-center justify-between font-medium transition-all outline-none ${
          isOpen ? "border-pro-blue-500 ring-1 ring-blue-500/50" : "border-pro-slate-700 hover:border-pro-slate-500"
        }`}
      >
            ...
            return (
              <div
                ...
                className={`flex items-center px-3 py-2.5 rounded-lg text-sm cursor-pointer select-none transition-colors ${
                  isChecked ? "bg-pro-blue-600/20 text-blue-400 font-bold" : "hover:bg-pro-slate-800 text-slate-300"
                }`}
              >
                <input
                  type="checkbox"
...
// ==========================================
// 3. 主頁面組件：球友目錄 (Bilingual & Slate Theme)
// ==========================================
function PlayerDirectoryComponent() {
  ...
  const mockPlayers: MockPlayer[] = [
    { id: "p1", name: "Alex Chen", avatar: "⚡", primarySport: "Tennis", skillLevel: "高階 Advanced", positionOrStyle: "底線型 / 慣用右手", district: "港島區 Island", lookingFor: "尋找平日晚上維園或中山紀念公園球友，可拉球一小時。", isAvailableThisWeekend: true },
    { id: "p2", name: "Sarah Wong", avatar: "🏐", primarySport: "Volleyball", skillLevel: "進階 Intermediate", positionOrStyle: "二傳手 (Setter)", district: "九龍區 Kowloon", lookingFor: "剛搬來九龍，想找週六下午的混排或女排歡樂場。", isAvailableThisWeekend: true },
    { id: "p3", name: "Marcus Lau", avatar: "🏀", primarySport: "Basketball", skillLevel: "校隊/專業 Pro", positionOrStyle: "控球後衛 (PG)", district: "新界區 N.T.", lookingFor: "全場 5v5 缺人可叫我，主攻組織與防守，不獨食。", isAvailableThisWeekend: false },
    { id: "p4", name: "David Pak", avatar: "🎾", primarySport: "Tennis", skillLevel: "進階 Intermediate", positionOrStyle: "發球上網型", district: "港島區 Island", lookingFor: "誠邀實力相當球友練習雙打走位。", isAvailableThisWeekend: true },
    { id: "p5", name: "Elena Rostova", avatar: "🏸", primarySport: "Badminton", skillLevel: "高階 Advanced", positionOrStyle: "單打 / 拉吊突擊", district: "九龍區 Kowloon", lookingFor: "尋找長期固定羽毛球夥伴，男女皆可，最重要準時。", isAvailableThisWeekend: true },
    { id: "p6", name: "Kenji Sato", avatar: "⚽", primarySport: "Football", skillLevel: "初學者 Beginner", positionOrStyle: "右後衛 (RB)", district: "港島區 Island", lookingFor: "新手求收留，體能好肯奔跑，主要想享受踢球樂趣。", isAvailableThisWeekend: false }
  ];

  const sportOptions: OptionItem[] = [
  ...
  const districtOptions: OptionItem[] = [
  ...
  const hasAnyFilter = selectedSports.length > 0 || selectedLevels.length > 0 || selectedDistricts.length > 0;

  const filterControlsJSX = (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
      <CustomMultiSelect title="運動項目 Sport" options={sportOptions} selected={selectedSports} onChange={setSelectedSports} />
      <CustomMultiSelect title="技術程度 Level" options={levelOptions} selected={selectedLevels} onChange={setSelectedLevels} />
      <CustomMultiSelect title="區域 District" options={districtOptions} selected={selectedDistricts} onChange={setSelectedDistricts} />
      <button 
        onClick={resetFilters}
        disabled={!hasAnyFilter}
        className={`w-full py-3 px-4 rounded-lg text-sm font-bold transition-all ${
          hasAnyFilter ? "bg-pro-slate-800 hover:bg-pro-slate-700 text-white" : "bg-pro-slate-900 text-slate-600 cursor-not-allowed border border-pro-slate-800"
        }`}
      >
        ↻ 重置條件 Reset
      </button>
  ...
  return (
    <div className="min-h-screen bg-pro-slate-950 text-slate-100 pb-24 font-sans">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-6">
        <Link href="/network" className="text-xs text-slate-500 hover:text-slate-300 transition-colors">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mt-4">
          <div>
            ...
            <p className="mt-2 text-slate-400 text-sm leading-relaxed">
              找到與你實力相當、出沒地區相同的運動夥伴。<br/>
              <span className="text-xs text-slate-500">Discover sports partners with similar skills in your area.</span>
            </p>
          </div>
          <div className="mt-4 md:mt-0 text-sm font-medium text-slate-500 bg-pro-slate-900 px-4 py-2 rounded-lg border border-pro-slate-800">
            符合條件 Matches：<span className="text-blue-500 font-bold text-base ml-1">{filteredList.length}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8">
        ...
        >
          ...
          {hasAnyFilter && <span className="w-2 h-2 rounded-full bg-pro-blue-500 ml-1"></span>}
        </button>
        ...
        {/* ============================================================== */}
        {/* 手機端底部抽屜 (Mobile Bottom Sheet Modal) */}
        {/* ============================================================== */}
        {isFilterOpen && (
          <div className="md:hidden fixed inset-0 z-50 flex items-end justify-center">
            ...
            <div className="relative w-full bg-pro-slate-950 border-t border-pro-slate-800 rounded-t-3xl p-6 pb-10 shadow-2xl animate-in slide-in-from-bottom-8 duration-300 ease-out max-h-[85vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setIsFilterOpen(false)} className="text-slate-400 hover:text-white p-2 text-sm font-bold bg-pro-slate-900 rounded-full w-8 h-8 flex items-center justify-center border border-pro-slate-800">
                  ✕
                </button>
              </div>
        ...
        {/* ============================================================== */}
        {/* 搜尋結果卡片網格 (Slate Theme & Bilingual) */}
        {/* ============================================================== */}
        {filteredList.length === 0 ? (
          <div className="text-center py-20 bg-pro-slate-900/40 rounded-2xl border border-dashed border-pro-slate-800 relative z-0">
            ...
            <h3 className="text-lg font-bold text-slate-300">找不到符合條件的球友 No matches found</h3>
            <button onClick={resetFilters} className="mt-4 text-xs font-bold text-blue-500 underline hover:text-blue-400">清除所有篩選 Clear all filters</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 relative z-0">
            {filteredList.map((player) => (
              <div key={player.id} className="bg-pro-slate-900 border border-pro-slate-800 rounded-2xl p-5 hover:border-pro-slate-700 transition-colors relative flex flex-col justify-between group shadow-sm">
                ...
                <div>
                  <div className="flex items-center gap-4 mb-4">
                    <span className="text-3xl bg-pro-slate-950 p-3 rounded-xl border border-pro-slate-800 shadow-inner">{player.avatar}</span>
                    <div>
                      <h3 className="font-bold text-white text-lg">{player.name}</h3>
                      <p className="text-xs font-medium text-blue-400 mt-0.5">
                        {player.primarySport} <span className="text-slate-500 font-normal ml-1">{player.skillLevel}</span>
                      </p>
                    </div>
                  </div>

                  <div className="bg-pro-slate-950 rounded-xl p-3 my-4 border border-pro-slate-800 shadow-inner">
                    <p className="text-[11px] font-mono text-slate-500 mb-1.5">戰術/位置 Style：{player.positionOrStyle}</p>
