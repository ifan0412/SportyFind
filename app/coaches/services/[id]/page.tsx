...
import { 
  ...
  Loader2, CheckCircle2, User as UserIcon, Send, Image as ImageIcon
} from "lucide-react";
  ...
  const handleSendInquiry = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("coach_enquiries").insert({
        service_id: serviceId, coach_id: service.coach_id, student_id: currentUser.id, message: inquireMsg.trim(),
      });
      if (error) throw error;
      alert("🎉 詢問單已成功發送！");
      setHasEnquired(true); setIsInquireOpen(false); setEnquiryCount(c => c + 1);
    } catch (err: any) { alert("發送失敗: " + err.message); } finally { setIsSubmittingInquiry(false); }
  };
  ...
  const handleAddReview = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("coach_reviews").insert({
        service_id: serviceId, coach_id: service.coach_id, student_id: currentUser.id, rating: ratingVal, comment: commentVal.trim(),
      });
      if (error) throw error;
      setCommentVal(""); await fetchServiceData();
    } catch (err: any) { alert("評價失敗: " + err.message); } finally { setIsSubmittingReview(false); }
  };

  if (isLoading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-zinc-500 font-mono"><Loader2 className="w-6 h-6 animate-spin mr-2 text-blue-500" /> 載入課程中...</div>;
  if (!service) return <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white p-4"><h2 className="text-xl font-bold mb-4">找不到此課程或服務</h2><button onClick={() => router.back()} className="text-blue-400 underline text-sm cursor-pointer">返回上一頁</button></div>;

  return (
    <div className="bg-slate-950 min-h-screen py-10 px-4 sm:px-6 lg:px-8 text-white">
      <div className="max-w-4xl mx-auto space-y-8">
        <button onClick={() => router.back()} className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white font-bold transition cursor-pointer"><ArrowLeft className="w-4 h-4" /> 返回教練檔案</button>

        {/* 課程詳細資訊主卡 */}
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <span className="px-3 py-1 rounded-full text-xs font-black bg-amber-500/10 text-amber-400 border border-amber-500/20 uppercase tracking-wider">{service.sport_category} 專業訓練</span>
            <span className="text-xs font-bold text-amber-400 bg-amber-950/50 border border-amber-500/30 px-3 py-1 rounded-full flex items-center gap-1.5">🔥 已有 {enquiryCount} 人發送諮詢單</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80 mb-6 text-sm">
            <div className="flex items-center gap-3"><MapPin className="w-5 h-5 text-amber-400 shrink-0" /><div><div className="text-xs text-zinc-500 font-bold">授課場地 / 地區</div><div className="font-extrabold text-white">{service.location || "可商議 / 到府或球場"}</div></div></div>
            <div className="flex items-center gap-3"><DollarSign className="w-5 h-5 text-emerald-400 shrink-0" /><div><div className="text-xs text-zinc-500 font-bold">課程報價 (HKD)</div><div className="font-extrabold text-emerald-400 text-base">${service.hourly_rate} <span className="text-xs text-zinc-400 font-normal">/ 每小時</span></div></div></div>
          </div>

          <div className="space-y-2 mb-8">
            <h3 className="text-xs font-black uppercase text-zinc-500 tracking-wider">課程與教學內容說明</h3>
            <p className="text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap bg-slate-950/30 p-4 rounded-2xl border border-slate-800/40">{service.description || "教練尚未填寫詳細課程說明。"}</p>
          </div>
          ...
          <div className="pt-6 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            {currentUser?.id !== service.coach_id && (
              <button onClick={() => setIsInquireOpen(true)} type="button" className={`w-full sm:w-auto px-8 py-3.5 rounded-xl font-black text-sm transition shadow-lg flex items-center justify-center gap-2 cursor-pointer active:scale-95 ${hasEnquired ? "bg-slate-800 text-zinc-300 border border-slate-700" : "bg-amber-600 hover:bg-amber-500 text-white"}`}>
        ...
        <div className="flex border-b border-slate-800 gap-6 px-2">
          <button
            ...
            type="button"
            className={`pb-3 text-base font-black transition flex items-center gap-2 cursor-pointer border-b-2 ${bottomTab === "reviews" ? "border-amber-500 text-white -mb-px" : "border-transparent text-zinc-500 hover:text-zinc-300"}`}
          >
          ...
          <button
            ...
            type="button"
            className={`pb-3 text-base font-black transition flex items-center gap-2 cursor-pointer border-b-2 ${bottomTab === "gallery" ? "border-amber-500 text-white -mb-px" : "border-transparent text-zinc-500 hover:text-zinc-300"}`}
          >
              ...
              <form onSubmit={handleAddReview} className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map(s => (
                      <button key={s} type="button" onClick={() => setRatingVal(s)} className={`text-base transition cursor-pointer ${ratingVal >= s ? "text-amber-400" : "text-zinc-600"}`}>★</button>
                    ))}
                  </div>
                </div>
                <textarea rows={2} placeholder="分享上課心得給其他運動員吧！" value={commentVal} onChange={(e) => setCommentVal(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-amber-500" />
                <div className="flex justify-end">
                  <button type="submit" disabled={isSubmittingReview} className="px-5 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 disabled:bg-slate-800 text-white font-bold text-xs transition cursor-pointer">{isSubmittingReview ? "送出中..." : "發佈評價"}</button>
                </div>
              </form>
            )}
              ...
              {reviews.length === 0 ? (
                ...
                reviews.map(rev => (
                  <div key={rev.id} className="pt-4 first:pt-0 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-slate-800 bg-cover bg-center" style={rev.student?.avatar_url ? { backgroundImage: `url(${rev.student.avatar_url})` } : undefined} />
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl">
            ...
            <form onSubmit={handleSendInquiry} className="space-y-4">
              <textarea rows={4} required placeholder="例：教練好！目前想加強接發球技術，請問週末有空檔嗎？" value={inquireMsg} onChange={(e) => setInquireMsg(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-amber-500" />
