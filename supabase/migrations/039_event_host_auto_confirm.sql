-- Event creators (hosts) are always auto-confirmed when registering for their own
-- individual events, including approval-mode events.

-- Fix existing host registrations stuck in pending/reviewing.
...
where er.event_id = e.id
  and er.user_id = e.creator_id
  and e.registration_type = 'individual'
  and lower(coalesce(er.status, '')) in ('pending', 'reviewing');
