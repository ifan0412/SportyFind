-- Waitlist promotion: host icon for joiner, leave-then-promote host notifications.
...
-- Notify host that someone left, then promote the next waitlisted player.
...
-- Leave + waitlist promotion are handled by the registration trigger (avoids duplicate/out-of-order notifs).
